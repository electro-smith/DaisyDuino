...
void myPCA9685::writeMode(uint8_t reg, uint8_t value)
{
  _error = PCA9685_OK;
...
uint8_t myPCA9685::readMode(uint8_t reg)
{
  _error = PCA9685_OK;
  if ((reg != PCA9685_MODE1) && (reg != PCA9685_MODE2))
  {
...
void myPCA9685::setPWM(uint8_t channel, uint16_t onTime, uint16_t offTime)
{
  _error = PCA9685_OK;
  if (channel > 15)
  {
...
// write value to single PWM channel
void myPCA9685::setPWM(uint8_t channel, uint16_t offTime)
{
...
void myPCA9685::getPWM(uint8_t channel, uint16_t* onTime, uint16_t* offTime)
{
  _error = PCA9685_OK;
  if (channel > 15)
  {
...
void myPCA9685::setFrequency(uint16_t freq, int offset)
{
  _error = PCA9685_OK;
  _freq = freq;
...
int myPCA9685::getFrequency(bool cache)
{
  _error = PCA9685_OK;
...
void myPCA9685::digitalWrite(uint8_t channel, uint8_t mode)
{
  _error = PCA9685_OK;
  if (channel > 15)
  {
...
// PRIVATE
//
void myPCA9685::writeReg(uint8_t reg, uint8_t value)
{
  _wire->beginTransmission(_address);
  _wire->write(reg);
  _wire->write(value);
...
void myPCA9685::writeReg2(uint8_t reg, uint16_t a, uint16_t b)
{
  _wire->beginTransmission(_address);
  _wire->write(reg);
  _wire->write(a & 0xFF);
...
uint8_t myPCA9685::readReg(uint8_t reg)
{
  _wire->beginTransmission(_address);
  _wire->write(reg);
  _error = _wire->endTransmission();
