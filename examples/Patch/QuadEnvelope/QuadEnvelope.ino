...
// Diagram: https://raw.githubusercontent.com/electro-smith/DaisyExamples/master/patch/QuadEnvelope/resources/QuadEnvelope.png
// Controls
//
...
struct envStruct
{
    ...
    void Process()
    {
        if (curveTimeMode == 0)
        {
            env.SetTime(ADENV_SEG_ATTACK, ((1023.f - analogRead(attackPin)) / 1023.f) + .01);
            env.SetTime(ADENV_SEG_DECAY, ((1023.f - analogRead(decayPin)) / 1023.f) + .01);
        }
        else
        {
...
void setup()
{
    ...
    envelopes[0].Init(samplerate, PIN_PATCH_CTRL_1, PIN_PATCH_CTRL_2, &patch.gateIns[0]);
    envelopes[1].Init(samplerate, PIN_PATCH_CTRL_3, PIN_PATCH_CTRL_4, &patch.gateIns[1]);
    curveTimeMode = 0;
